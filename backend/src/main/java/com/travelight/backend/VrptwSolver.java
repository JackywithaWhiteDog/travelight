...
public class VrptwSolver {
    ...
    public static void solve(VrptwDataModel vrptwDataModel) {
        ...
        RoutingIndexManager manager =
            new RoutingIndexManager(vrptwDataModel.timeMatrix.data.length, vrptwDataModel.vehicleNumber, vrptwDataModel.depot);
        ...
        for (int i = 1; i < vrptwDataModel.timeWindows.data.length; ++i) {
          ...
          timeDimension.cumulVar(index).setRange(vrptwDataModel.timeWindows.data[i][0], vrptwDataModel.timeWindows.data[i][1]);
        }
        // Add time window constraints for each vehicle start node.
        int vehicle = 0;
        long index = routing.start(vehicle);
        timeDimension.cumulVar(index).setRange(vrptwDataModel.timeWindows.data[0][0], vrptwDataModel.timeWindows.data[0][1]);
    ...
    public static void printSolution(VrptwDataModel vrptwDataModel, RoutingModel routing, RoutingIndexManager manager, Assignment solution) {
        // Inspect solution
        RoutingDimension timeDimension = routing.getMutableDimension("Time");
