        ...
        if (response.openingHours == null) {
            ...
            for (int i = 0; i < periods.length; i++) {
                ...
                if (p.open == null) {
                    ...
                    openTime = Double.valueOf(p.open.time.getHour()) + Double.valueOf(p.open.time.getMinute()) / 60;
                }

                if (p.close == null) {
                    closeTime = 24.0;
                } else {
                    closeTime = (p.open.day.getName().equals(p.close.day.getName()))
                            ? Double.valueOf(p.close.time.getHour()) + Double.valueOf(p.close.time.getMinute()) / 60
                            : 24.0;
                }
                openingTimes[day] = (openingTimes[day] == -1) ? openTime : Math.min(openingTimes[day], openTime);
                closingTimes[day] = (closingTimes[day] == -1) ? closeTime : Math.max(closingTimes[day], closeTime);
            }
