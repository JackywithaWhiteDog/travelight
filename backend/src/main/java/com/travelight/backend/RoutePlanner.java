...
public class RoutePlanner {
    ...
    public static TravelSchedule planTravelSchedule(OptimizationInfo optimizationInfo) {
        ...
        List<Integer> originalOrder = IntStream.rangeClosed(1, optimizationInfo.attractions.length).boxed().collect(Collectors.toList()); // original travel order
        if (performCheck) {
    ...
    public static TravelSchedule optimizeOrderBruteForce(VrptwDataModel vrptwDataModel, List<Integer> originalOrder) {
        ...
        if (originalTravelSchedule.isValid) {
            ...
            System.out.println("\nThe original transporation time: " + originalTransportationTime + " / " + "The best transportation time: " + minTransportationTime + " / " + "Total saved time: " + savedTime);
    ...
    public static TravelSchedule checkOrder(VrptwDataModel vrptwDataModel, List<Integer> order) {
        ...
        long curTime = vrptwDataModel.timeWindows.data[order.get(numAttractions - 1)][1];; // initial time @ the last arriveTime
        for (int i = numAttractions - 1; i >= 0; i--) {
            ...
            if (curTime < openingTime) {
                ...
                if (closingTime < curTime) {
                    ...
                    curTime = closingTime; // need to stay idle until the travaller can leave for the next attraction
                }
    ...
    public static TimeWindows getTimeWindowsFromAttractions(Attraction[] attractions, int departureDay) {
        long[][] data = new long[attractions.length + 1][2]; // +1: dummy row, 2: openingTime & closingTime
        // dummy depot
        data[0][0] = 0;
